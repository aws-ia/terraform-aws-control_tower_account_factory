...
def check_workspace_exists(organization_name, workspace_name, api_token):
    endpoint = "{}/organizations/{}/workspaces/{}".format(TERRAFORM_API_ENDPOINT, organization_name, workspace_name)
...
def create_workspace(organization_name, workspace_name, api_token):
    ...
    if workspace_id:
        ...
        endpoint = "{}/organizations/{}/workspaces".format(TERRAFORM_API_ENDPOINT, organization_name)
        headers = __build_standard_headers(api_token)
        payload = {
            "data": {
                "attributes": {
...
def create_configuration_version(workspace_id, api_token):
    endpoint = "{}/workspaces/{}/configuration-versions".format(TERRAFORM_API_ENDPOINT, workspace_id)
    headers = __build_standard_headers(api_token)
    payload = {
        "data": {
...
def set_environment_variable(key, value, description, workspace_id, sensitive, category, api_token):
...
def update_environment_variable(var_id, key, value, description, workspace_id, sensitive, category, api_token):
    endpoint = "{}/workspaces/{}/vars/{}".format(TERRAFORM_API_ENDPOINT, workspace_id, var_id)
    headers = __build_standard_headers(api_token)
    payload = {
        "data": {
            "attributes": {
...
def create_destroy_run(workspace_id, api_token):
    ...
    payload = {
        "data": {
            "attributes": {
                "is-destroy": True,
                "message": "Destroy run created by AFT"
            },
            "type": "runs",
            "relationships": {
                "workspace": {
...
def __handle_errors(response):
    ...
    if len(errors) == 0:
        ...
        print("More than one error returned by client; raising internal failure and placing all errors in the message")
