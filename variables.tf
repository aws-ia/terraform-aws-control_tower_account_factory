...
variable "aft_vpc_nat_gateway" {
  type        = bool
  description = "Flag turning NAT Gateway on/off for AFT VPC"
  default     = true
  validation {
    condition     = contains([true, false], var.aft_vpc_nat_gateway)
    error_message = "Valid values for var: aft_vpc_nat_gateway are (true, false)."
  }
}
