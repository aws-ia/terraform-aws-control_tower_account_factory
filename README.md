...
| <a name="input_aft_vpc_nat_gateway"></a> [aft\_vpc\_aws\_nat\_gateway](#input\_aft\_vpc\_aws\_nat\_gateway) | Flag turning NAT GW on/off and all the resources related. Set it off to reduce costs when no account request are send.| `bool` | `true` | no |
