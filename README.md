...
| <a name="input_create_ssm_block_public_sharing"></a> [create\_ssm\_block\_public\_sharing](#input\_create\_ssm\_block\_public\_sharing) | Whether to block public sharing of SSM documents in the AFT management account. Addresses Security Hub control SSM.7. | `bool` | `true` | no |
...
| <a name="input_ssm_block_public_sharing_regions"></a> [ssm\_block\_public\_sharing\_regions](#input\_ssm\_block\_public\_sharing\_regions) | Control Tower governed regions in which to block public sharing of SSM documents in the AFT management account. The setting is account-level but evaluated per region, so list every governed region to fully remediate Security Hub control SSM.7. The Control Tower home region is always included. | `list(string)` | `[]` | no |
