...
def add_header(request: Any, **kwargs: Any) -> None:
    request.headers.add_header("User-Agent", "account-factory-terraform-" + aft_version)  # type: ignore
...
def lambda_handler(event: Dict[str, Any], context: Union[Dict[str, Any], None]) -> None:
    ...
    try:
        ...
        aft_version = utils.get_ssm_parameter_value(  # type: ignore
            session, "/aft/config/aft/version"
        )
