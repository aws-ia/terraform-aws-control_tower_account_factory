    ...
    if tags:
        ...
        for tag in tags:
            ...
            if is_valid_semver:
                ...
                version_parts_ints = map(lambda version: int(version), version_parts)
        ...
        incremented_patch_version = (
            latest_semantic_version_parts[patch_version_index] + 1
        )
...
def print_struct(data: str, indent: str = ""):
    print(
        f"{indent}"
        + pprint.pformat(data).replace("'", '"').replace("\n", f"\n{indent}")
    )


def print_ssm_parameter_to_write(
    name: str, account_id: str, current_region: str, value: str, indent: str
):
...
def _subprocess_call(cmd: str, no_echo: bool = False):
    ...
    if len(stdout) + len(stderr) > 0:
        ...
        # - git add -f examples/workspace/terraform.auto.tfvars
        if re.search(
            "(.*added/updated.*|create mode|file changed,|master -> .*master|terraform.auto.tfvars)",
            output,
        ):
            output = cyan(output)
    ...
    if process.returncode != 0:
        output = red(
            f"    {indent}ERROR: shell command '{cmd}' failed with return code '{process.returncode}'"
        )
        result = f"{result}\n{output}"
