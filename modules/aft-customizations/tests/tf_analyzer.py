...
import os
import pprint
from common import red, cyan, yellow, load_json
from common import subprocess_call, subprocess_call_output, subprocess_call_fails
...
default_config = {
    "resource_categories" : ["iam" , "vpc", "lambda", "db","config","ssm","cloudwatch","event","null_resource"],
...
class tfAnalyzer:
    ...
    def get_modules(self):
        ...
        if "child_modules" in self._plan["planned_values"]["root_module"].keys():
            root_module = self._plan["planned_values"]["root_module"]["child_modules"][0]
            ...
            for r in results[m]["resources"]:
                ...
                if r["type"].split("_")[-1] in r["name"]:
                    ...
                    if not "RN01" in by_violations.keys():
                        by_violations["RN01"] = []
                    by_violations["RN01"].append({ "resource": r, "msg": f'{r["name"]} has {r["type"].split("_")[-1]} in the name'})
                ## check-GN01 violations
                if "-" in r["name"]:
                    logger.debug(f"GN01 --> {r['name']} : {r['type']}")
                    if not "GN01" in by_violations.keys():
                        by_violations["GN01"] = []
                    by_violations["GN01"].append({ "resource": r, "msg": f"{r['name']} has - in the name"})
                ## check-GN02 violations
                if "values" in r and r["type"] not in tf_resource_types_without_named_aws_resources:
                    key = "name"
                    #print(r)
                    if r["type"] in name_keys_map.keys():
                        key = name_keys_map[r["type"]]
                    if key not in r["values"]:
                        logger.warning(f"WARN: {key} does not exists in values for {r['type']}")
                    else:
                        aws_resource_name = r["values"][key]
                        if aws_resource_name:
                            if "_" in aws_resource_name:
                                logger.debug(f"GN02: violation for {aws_resource_name} ==> {r['name']}, {r['type']}")
                                if not "GN02" in by_violations.keys():
                                    by_violations["GN02"] = []
                                by_violations["GN02"].append({ "resource": r, "msg": f"{aws_resource_name} has _ in the aws_resource name"})
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option("-f", "--plan_file", dest="plan_file", help="terraform plan in json format")
    ...
    table = [[m, len(modules[m]["child_modules"]),len(modules[m]["resources"])] for m in modules]
